...
export default function APIPage() {
  ...
  const totalSetup    = clients.reduce((s, c) => s + c.setup, 0)
  const totalBudget   = clients.reduce((s, c) => s + c.budget, 0)
  const avgMinBilling = Math.round(clients.reduce((s, c) => s + c.min_billing, 0) / clients.length)
  ...
  const metrics = [
    ...
    { label: 'Min. facturation moyen',   value: avgMinBilling + ' €',                        color: '#13d275' },
  ...
  function exportCSV() {
    ...
    const rows = [keys.join(','), ...payload.map(r => keys.map(k => JSON.stringify(r[k])).join(','))].join('\n')
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-xl border border-border overflow-hidden mb-4">
        <div className="px-4 py-3 border-b border-border text-[10px] font-bold text-info uppercase tracking-wide">Données clés par client</div>
          ...
          <tbody>
            {clients.map(c => (
              <tr key={c.id} className="border-b border-border hover:bg-bg">
                ...
                <td className="px-4 py-2.5">{c.setup.toLocaleString('fr-FR')} €</td>
                <td className="px-4 py-2.5">{c.min_billing.toLocaleString('fr-FR')} €</td>
                <td className="px-4 py-2.5">{c.budget.toLocaleString('fr-FR')} €</td>
